...
?>				  
							...
							<li class="galleryItem isotope-item album-1 ">	
								...
								<a class="image-wrap" href="home.php">
									<img class="img-border" src="<?php echo $row['Image']; ?>" alt="Album 1">
							...
							<input type="button" Value="+" name="add" 
											...
											onclick="var effect = document.getElementById('qty<?php echo $row['C_id']; ?>');
											var qty = effect.value; 
											if( !isNaN( qty )) effect.value++;
											addcart(<?php echo $row['Productmaster_id']?>,'p');
											return false;"
											/>	
							
							<input type="button" name="qty" style="width:50px;" id="qty<?php echo $row['C_id']; ?>" maxlength="12" value="<?php echo $row['Quantity_cart']; ?>" title="Quantity:" />
							...
							<input type="button" Value="-" name="sub"
											...
											rmcart(<?php echo $row['Productmaster_id']?>,'m');
											return false;"
											/>
							...
							<li class="galleryItem isotope-item album-1 ">	
									...
									<h5>
										<?php echo $row['Quantity_cart']*$row['Price']."₹"; 
											$tot+=$row['Quantity_cart']*$row['Price'];
										?>	
...
function addcart(str,qty) {
  ...
  xhttp.open("GET", "addtocart.php?Productmaster_id="+str+"&Quantity_cart="+qty, true);
  xhttp.send();

  location.reload();
}

function rmcart(str,qty) {
  var xhttp;   
  ...
  xhttp.open("GET", "rmcart.php?Productmaster_id="+str+"&Quantity_cart="+qty, true);
  xhttp.send();
  ...
  location.reload();
				  ...
				  <div class="clear"></div>
				  	<a href="index.php"><button type="submit" name="pay">Checkout</button> </a>
...
<section id="before_footer">
    <div class="container clearfix">
        <div class="row">  
            <div class="span12">  
                <div id="text-5">			
                	<div class="textwidget"><div id="ribbon">Give your child professional basic education!</div></div>
				</div>    
            </div>
        </div> 
    </div>       
